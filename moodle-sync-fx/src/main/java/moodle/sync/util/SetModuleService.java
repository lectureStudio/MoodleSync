...
public final class SetModuleService {
    ...
    public static void publishResource(MoodleService moodleService, syncTableElement courseData, Course course, String url , String token) throws Exception {
        if (courseData.getAction() == MoodleAction.MoodleUpload) {
                ...
                MoodleUpload upload = uploader.upload(courseData.getExistingFileName(), courseData.getExistingFile(), url, token);
                //Publish it in the Moodle-course.
                if (courseData.getUnixTimeStamp() > System.currentTimeMillis() / 1000L) {
                    //Time in future
                    moodleService.setResource(token, course.getId(), courseData.getSection(), upload.getItemid(), courseData.getUnixTimeStamp(), courseData.getVisible(), courseData.getModuleName(), courseData.getBeforemod());
                } else {
                    //Time not modified, time should be null
                    moodleService.setResource(token, course.getId(), courseData.getSection(), upload.getItemid(), null, courseData.getVisible(), courseData.getModuleName(), courseData.getBeforemod());
                }
    ...
    public static void publishFileserverResource(MoodleService moodleService, syncTableElement courseData, Course course, String token) throws Exception {
        //TODO: konkreter fileserver upload hinzufügen
        // url = fileservice.....
        String url = "https://wikipedia.org";
        if (courseData.getUnixTimeStamp() > System.currentTimeMillis() / 1000L) {
            moodleService.setUrl(token, course.getId(), courseData.getSection(), courseData.getModuleName(), url, courseData.getUnixTimeStamp(), courseData.getVisible(), courseData.getBeforemod());
        } else {
            moodleService.setUrl(token, course.getId(), courseData.getSection(), courseData.getModuleName(), url, null, courseData.getVisible(), courseData.getBeforemod());
        }
    }
    ...
    public static void createSection(MoodleService moodleService, syncTableElement courseData, Course course, String token) throws Exception {
